...
def load_library(filename='library.txt'):
    ...
    try:
        with open(filename, 'r') as f:
            ...
            for line in f:
                line = line.strip()
                if line == "---":  # End of a book entry
                    if current_book:
                        # Convert data types and add to the library
                        try:
                            current_book['publication_year'] = int(current_book['publication_year'])
                            current_book['read_status'] = current_book['read_status'] == 'True'
                        except (KeyError, ValueError):
                            print("Invalid data in file. Skipping book.")
                            current_book = {}
                            continue
            ...
            # Add the last book if the file doesn't end with "---"
            if current_book:
                try:
                    current_book['publication_year'] = int(current_book['publication_year'])
                    current_book['read_status'] = current_book['read_status'] == 'True'
                except (KeyError, ValueError):
                    print("Invalid data in last book. Skipping.")
                else:
                    books.append(current_book)
    except FileNotFoundError:
        pass  # File doesn't exist yet; start with empty library
    return books
...
def save_library(books, filename='library.txt'):
    with open(filename, 'w') as f:
        for book in books:
            # Write each book's details as key-value pairs
            f.write(f"title: {book['title']}\n")
            f.write(f"author: {book['author']}\n")
            f.write(f"publication_year: {book['publication_year']}\n")
            f.write(f"genre: {book['genre']}\n")
            f.write(f"read_status: {book['read_status']}\n")
